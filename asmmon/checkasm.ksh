...
#Default source is oem
SOURCE=oem

while getopts "c:m:s:" arg
do
  case $arg in
  m)
     ...
     if [[ "$OPTARG" = 'TEAM' ]];then
       ...
       EMAIL_LIST="emily.beswick@ihsmarkit.com,Alistair.Thompson@ihsmarkit.com,shivdeep.modi@ihsmarkit.com"
       ASM_CHECK_FILE=${WORK_DIR}/asmmon_all.sql
       MAIL_SUBJECT='ASM Diskgroup Report (ALL)'
     else
       EMAIL_LIST=shivdeep.modi@ihsmarkit.com
       MAIL_SUBJECT='ASM Diskgroup ALERT'
     fi
  ...
  s) 
     ...
     if [[ "$OPTARG" = 'oem' ]];then
       ...
       ASM_CHECK_FILE=${WORK_DIR}/asmmon_connect_to_host.sql
       SOURCE=host
     fi
    ;;
  *) 
...
echo "`date +'%d-%h-%y %H:%M:%S'` : Host section" >> $TMP_LOG
