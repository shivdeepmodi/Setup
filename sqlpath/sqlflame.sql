...
SET HEADING OFF LINESIZE 32767 PAGESIZE 0 TRIMSPOOL ON TRIMOUT ON LONG 9999999 VERIFY OFF LONGCHUNKSIZE 100000 FEEDBACK OFF APPINFO OFF TIMING OFF
PROMPT
...
SELECT
    ...
    '0 - SELECT STATEMENT'||TRIM(SYS_CONNECT_BY_PATH(id||' - '||operation||NVL2(options,' '||options,NULL)||NVL2(object_owner||object_name, ' ['||object_owner||'.'||object_name||']', NULL), ';'))||' '||TRIM(ROUND(self_elapsed_time/1000,2))
FROM
    combined
CONNECT BY
    parent_id = PRIOR id 
...
spool /tmp/sqlflame_stacks.txt
/
spool off

SET TERMOUT ON HEADING ON PAGESIZE 5000 LINESIZE 999 FEEDBACK ON  TIMING ON

HOST /home/oracle/shivdeep/FlameGraph/flamegraph.pl --countname=Milliseconds --title="sql_id=&1" /tmp/sqlflame_stacks.txt > /tmp/sqlflame_&1..svg
