...
declare
baseline_ref_cursor DBMS_SQLTUNE.SQLSET_CURSOR; min_snap number; max_snap number; ret binary_integer; l_sql_handle varchar2(40); l_plan_name varchar2(40); l_old_plan_name varchar2(40); major_release varchar2(3); minor_release varchar2(3); BEGIN select regexp_replace(version,'\..*'), regexp_substr(version,'[0-9]+',1,2) into major_release, minor_release from v$instance;

select min(snap_id), max(snap_id) into min_snap, max_snap from dba_hist_snapshot;

open baseline_ref_cursor for
select VALUE(p) from table(DBMS_SQLTUNE.SELECT_WORKLOAD_REPOSITORY(min_snap, max_snap, 'sql_id='||CHR(39)||'&&sql_id'||CHR(39)||' and plan_hash_value=&plan_hash_value',NULL,NULL,NULL,NULL,NULL,NULL,'ALL')) p;

DBMS_SQLTUNE.LOAD_SQLSET('CREATE_BASELINE_AWR', baseline_ref_cursor);

ret := DBMS_SPM.LOAD_PLANS_FROM_SQLSET ( sqlset_name => 'CREATE_BASELINE_AWR', sqlset_owner => 'RSHARMA', fixed => 'NO', enabled => 'YES');
...
dbms_output.put_line('Baseline '||upper(l_plan_name)||' created.'); dbms_output.put_line(' ');

end;
/
/*
set feedback on
select NAME,OWNER,CREATED,STATEMENT_COUNT FROM DBA_SQLSET where name like 'CREATE_BASELINE_AWR'; select spb.sql_handle, spb.plan_name, spb.sql_text, spb.enabled, spb.accepted, spb.fixed, to_char(spb.last_executed,'dd-mon-yy HH24:MI') last_executed from dba_sql_plan_baselines spb where upper(spb.plan_name) = upper('&&plan_name'); */ 
clear breaks 
exec  DBMS_SQLTUNE.DROP_SQLSET( sqlset_name => 'CREATE_BASELINE_AWR' ); 
undef sql_id undef plan_hash_value undef fixed undef enabled undef plan_name
