...
select a.snap_id,cast(b.BEGIN_INTERVAL_TIME as date) snapshot_time,a.sql_id, 
 ...
 where sql_id in (select sql_id from dba_hist_sqltext where sql_text like 'select TRACKINGID, LOGID, UUID, LOGTYPE, LOGTIMESTAMP, ACTIVITYNAME%')
 )
 and &&clause
 order by 2,3
/
...
--         ,a.OPTIMIZER_COST
 ...
 and a.sql_id in (
 select sql_id from dba_hist_sqltext 
 --where upper(sql_text) like 'select sql_id from dba_hist_sqltext where sql_text like 'INSERT /*+ APPEND */ INTO STG_FTB_TRD_VOL SELECT A%FROM VW_SETL_NON_DOGS_NON_CNS A WHERE  cob_dt%IOWA%IOWAAU%'
 where sql_id in (select sql_id from dba_hist_sqltext where sql_text like 'select TRACKINGID, LOGID, UUID, LOGTYPE, LOGTIMESTAMP, ACTIVITYNAME%')
 )
